...
from app.memory.vector_store_pg import get_last_curator_snapshot
...
def _llm_generate_questions(client: OpenAI, topics: List[str], weaknesses: List[str], count: int) -> List[Dict[str, Any]]:
    ...
    prompt = (
        "Сгенерируй тестовые вопросы (множественный выбор, 4 опции) по темам и слабым местам студента.\n"
        "Ответ строго JSON:\n"
        "{ \"questions\": ["
...
def generate_exam(count: int = 5, student_id: str = "default") -> Dict[str, Any]:
    ...
    2) Пытается сгенерировать вопросы через LLM.
    3) При любой ошибке — детерминированный fallback (не пустой).
    """
    ...
    # если вообще нет данных — дадим тренировочный набор
    if not topics and not weaknesses:
        return {
            "ok": True,
            "questions": _fallback_questions(["базовые понятия"], [], count),
            "rubric": "1 балл за правильный ответ. Фокус — базовые понятия."
        }

    client = _llm()
    if client:
        try:
            qs = _llm_generate_questions(client, topics, weaknesses, count)
    ...
    # fallback всегда непустой и валидный
